...
Display model: the overview shows all four courses as quantified work footprints, then shows the shared 12-gate rail for the active course. Course pages map existing work to its exact credit home. Unlike units are never summed, and mapped evidence earns no gate credit without a course-approved closure receipt.
