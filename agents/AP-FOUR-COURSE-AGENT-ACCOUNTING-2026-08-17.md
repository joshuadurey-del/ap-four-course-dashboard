...
| Orchestrator machine (Josh's) | Josh only | The few machine-bound artifacts are named below with a durable-copy plan; access itself is the documented constraint |
...
| `JOSH_MACHINE_ONLY` | Named local artifact. A durable-copy plan is stated. |
...
```json
{
  ...
  "claims": [
    {"claim_id": "humgeo.rebuild.passes", "section": "HumGeo", "status": "OBSERVED", "observed_at": "2026-08-17T13:20+09:00", "value": "432/474 slots hold current-byte official /v1/qc passes", "scope": "regeneration campaign slots only", "excluded": ["6 metadata rows", "759-line structural cohort"], "evidence": [{"type": "state-snapshot", "location": "orchestrator machine: issue21-regeneration-state-474.json", "durable": "seal manifest publishes the final counts post-drain"}, {"type": "git-branch", "url": "https://github.com/joshuadurey-del/ap-ss-evidence/tree/evidence/humgeo/20260816-004715__claude__humgeo--adoption-verifications/claude", "commit": "b6f5f1e8f585633f16f6733f8b9b315a9426d3ac"}], "evidence_access": "ILMA_HUMAN_ONLY+JOSH_MACHINE_ONLY"},
    {"claim_id": "humgeo.residual.draining", "section": "HumGeo", "status": "OBSERVED", "observed_at": "2026-08-17T13:20+09:00", "value": "residual runner alive (13h45m up). 100/142 slots complete, zero corpus writes. Log quiet since 12:45 KST — retry backoff or a slot spin; the watchdog is armed on exit either way. 36 blocked-slot events so far, mostly untyped 502s, all covered by the amended relaunch", "scope": "runner log tail + process table", "excluded": [], "evidence": [{"type": "log", "location": "orchestrator machine: issue21-residual-v3-20260816.log"}], "evidence_access": "JOSH_MACHINE_ONLY"},
    ...
    {"claim_id": "humgeo.metadata6.qc", "section": "HumGeo", "status": "PLANNED", "value": "6 metadata rows through official /v1/qc; pre-POST intents written", "evidence": [{"type": "intents", "location": "orchestrator machine: pre-post-intents-20260817.json"}], "evidence_access": "JOSH_MACHINE_ONLY"},
    ...
    {"claim_id": "apwh.skillcodes", "section": "APWH", "status": "OBSERVED", "observed_at": "2026-08-17T13:21+09:00", "value": "760/933 skill codes resolved. Redraw round 2: 61 of 207 done, ~56% recovery. Repair wave relaunched 13:21 to consume codes as they land", "evidence": [{"type": "seat-cursor", "location": "APWH campaign seat, live read"}], "evidence_access": "JOSH_MACHINE_ONLY"},
    {"claim_id": "apwh.regen_tail", "section": "APWH", "status": "PLANNED", "value": "362-item regeneration tail (260 multi-check + 66 content-class + 36 S10); dry-run receipt 362/362 OBSERVED; execution parked on HumGeo pool release", "evidence": [{"type": "dry-run", "location": "APWH seat day-dir"}], "evidence_access": "JOSH_MACHINE_ONLY"},
    ...
    {"claim_id": "psych.articles", "section": "Psych", "status": "OBSERVED", "observed_at": "2026-08-17", "value": "221/221 articles complete, every EK covered (U5's 54 landed 2026-08-16)", "evidence": [{"type": "git-branch", "url": "https://github.com/joshuadurey-del/ap-ss-evidence/tree/evidence/psych/20260817-local-host/claude", "commit": "054e5c286696bb89dab2d7148960634de6ef3e07"}], "evidence_access": "ILMA_HUMAN_ONLY"},
    {"claim_id": "psych.topic33.empty", "section": "Psych", "status": "OBSERVED", "observed_at": "2026-08-17", "value": "topic 3.3 (Gender and Sexual Orientation) carries 0 lessons BY DESIGN: no EK record maps to 3.3 (coverage invariant, course repo DOWNSTREAM_NOTES section 1); an approved topic-3.3 lesson asset exists on the shelf; a 2026-08-09 ruling parked adding it as a separate curriculum-authoring decision; verified today: exactly one empty topic in the served tree", "scope": "served course tree", "excluded": [], "evidence": [{"type": "repo-doc", "location": "Psych course repo DOWNSTREAM_NOTES section 1"}], "evidence_access": "JOSH_MACHINE_ONLY"},
    {"claim_id": "psych.local_serving", "section": "Psych", "status": "OBSERVED", "observed_at": "2026-08-17", "value": "boot-on-demand serving proven: the real BFF app booted in-process over the assembled bundle (262 components); 221/221 article student-views served; tree exact; zero key leaks; 246 receipts; reproducible on one documented command (WALK-SUMMARY.md in the evidence branch)", "scope": "article-serving paths only", "excluded": ["gate paths", "practice paths — bank-gated, unmeasured"], "evidence": [{"type": "git-branch", "url": "https://github.com/joshuadurey-del/ap-ss-evidence/tree/evidence/psych/20260817-local-host/claude", "commit": "054e5c286696bb89dab2d7148960634de6ef3e07"}], "evidence_access": "ILMA_HUMAN_ONLY"},
    {"claim_id": "psych.bank.shortfall", "section": "Psych", "status": "OBSERVED", "observed_at": "2026-08-17T09:00+09:00 (admin dry_run)", "value": "1,810 items short across 374 buckets (221 evidence-based-FRQ + 147 article-analysis + 6 MCQ)", "evidence": [{"type": "admin-dry-run", "location": "Psych seat day-dir"}], "evidence_access": "JOSH_MACHINE_ONLY"},
    ...
    {"claim_id": "psych.ebq.zero_yield", "section": "Psych", "status": "OBSERVED", "observed_at": "2026-08-11", "value": "evidence-based-FRQ generation produced 0 usable items in 10 passes; fresh measurement is PLANNED on the first two fill cycles; 221 of the 374 short buckets need this generator", "evidence": [{"type": "seat-receipts", "location": "Psych seat, 2026-08-11 generation logs"}], "evidence_access": "JOSH_MACHINE_ONLY"},
    {"claim_id": "psych.u5gate", "section": "Psych", "status": "BLOCKED", "blocker": "fleet (repair assigned)", "value": "U5 acceptance gate: 27 findings, unchanged since 2026-08-12", "evidence": [{"type": "github-issue", "url": "https://github.com/InceptTrilogy/ap-psychology-fall-2025-v1/issues/2"}], "evidence_access": "SHARED"},
    ...
    {"claim_id": "cross.judge.retired", "section": "Cross-course", "status": "OBSERVED", "observed_at": "2026-08-16 (measurement) / 2026-08-17T12:11+09:00 (retirement ruling)", "value": "local pre-check judge measured against official verdicts: 35 pairs x 3 votes, 105 calls, 0 errors; best fail-recall 80% strict / 70% majority; never gated anything; owner turned it off entirely on 2026-08-17", "scope": "35-pair sample", "excluded": [], "evidence": [{"type": "lane-log", "location": "orchestrator machine: humgeo-primary-orchestrator.log (ruling entry 12:11 KST)"}], "evidence_access": "JOSH_MACHINE_ONLY"},
    {"claim_id": "cross.judge.attribution", "section": "Cross-course", "status": "MISSING", "value": "which model/settings served official /v1/qc content checks on 2026-08-15 — receipts omit the judging model; 10 of 18 locally-missed fails concentrate in D2 (distractor plausibility) on DOK-1 recall items; fleet attribution requested", "evidence": [], "evidence_access": "NONE_YET"}
  ]
}
```
...
- Source: campaign state snapshot `issue21-regeneration-state-474.json` (orchestrator machine); each accepted slot carries a QC receipt bound to the item's byte fingerprint.
- Verify now: not independently verifiable off-machine until the seal; the adoption-time verification of the then-425 passes is published at [adoption-verifications](https://github.com/joshuadurey-del/ap-ss-evidence/tree/evidence/humgeo/20260816-004715__claude__humgeo--adoption-verifications/claude) (`b6f5f1e8f585633f16f6733f8b9b315a9426d3ac`).
...
**cross.judge.attribution** · `MISSING`
- The QC receipts do not record which model/settings served the official content checks on 2026-08-15. This absence is the finding. The ask to the fleet: name the model + settings; that one line either explains the D2 strictness gap or rules it out.
